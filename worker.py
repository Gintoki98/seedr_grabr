...
Por cada item encolado (torrent que hizo match con un filtro del usuario):
  1. Espera a que haya espacio suficiente en la cuenta de Seedr (con margen).
  2. Agrega el torrent (POST /tasks) a partir de la URL tomada de <guid>.
  3. Espera a que Seedr termine de descargarlo (polling de /tasks/{id},
     luego /tasks/{id}/contents para obtener los archivos resultantes).
  4. Descarga cada archivo resultante a disco local (GET /download/file/{id}).
  5. Borra cada archivo en Seedr ni bien está en local (libera espacio).
  6. Sube el archivo a Telegram (canal/grupo configurado) vía Telethon.
...
import config
import db
from seedr_auth import TokenStore, get_valid_access_token
...
def _get_seedr_client(token_store: TokenStore) -> SeedrClient:
...
async def _process_item(
    ...
    try:
        client = await asyncio.to_thread(_get_seedr_client, token_store)
        ...
        # 2) Agregar el torrent.
        await asyncio.to_thread(db.update_item_status, item_id, "adding_torrent")
        add_resp = await asyncio.to_thread(client.add_torrent_from_page, torrent_url, 0)
        ...
        for f in files:
            ...
            # 5) Borrar el archivo de Seedr ni bien está en local, para liberar espacio.
            try:
...
async def worker_loop(
    ...
    while not stop_event.is_set():
        ...
        try:
            await _process_item(item_id, token_store, telegram_client, stop_event)
        finally:
